class PanelController < ApplicationController
  ...
  def index
    ...
    if params.key?(:filt_fo) && params.key?(:filt_v)
      ...
      @filter_query = campos.zip(ops,vals).map {|a| (@models.columns_hash[a[0]].type == :text ? ("lower(unaccent(" + a[0] + "))") : a[0]) + a[1] + (a[1] == " ilike " ? ("'%" + a[2].mb_chars.normalize(:kd).gsub(/[^\x00-\x7F]/n,'').downcase.gsub('%',"¸%").gsub("_","¸_") + "%' escape '¸'") : (a[1] == " = " ? ("'" + a[2].downcase.mb_chars.normalize(:kd).gsub(/[^\x00-\x7F]/n,'').gsub("\\","\\\\") + "'") : (@models.columns_hash[a[0]].type == :date ? ("to_date('" + a[2] + "','YYYY-MM-DD')") : a[2]))) }.join(" AND ")
    end
...
#        end
  ...
  def query(keyword)
    ...
    (@models.columns_hash.keys - ["id", "created_at", "updated_at"]).each do |k|
      or_query = or_query + (or_query != "(" ? " OR " : "") + "lower(unaccent(text(" + k + "))) ilike '%" + keyword.mb_chars.normalize(:kd).gsub(/[^\x00-\x7F]/n,'').downcase.gsub('%',"¸%").gsub("_","¸_") + "%' escape '¸'"
    end
    return or_query + ")"
  end
  ...
  def obj_params
    ...
    params.require(@models.to_s.underscore.to_sym).permit(campos, @checks.keys.map{|k| [k,[]]}.to_h)
  end
end
